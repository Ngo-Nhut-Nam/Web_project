...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt_res->execute();
    $result_id = $stmt_res->insert_id; // Lấy ID vừa insert
    ...
    foreach ($questions as $q) {
        ...
        // Lưu chi tiết (có thể tạo bảng result_detail nếu muốn lưu chi tiết từng câu)
        // Nếu chưa có bảng result_detail, bạn có thể bỏ qua phần này
        /*
        $stmt_detail = $conn->prepare("INSERT INTO results_detail (result_id, question_id, user_answer) VALUES (?, ?, ?)");
        $stmt_detail->bind_param("iis", $result_id, $q['question_id'], $userAnswer);
        $stmt_detail->execute();
        */
    ...
    echo '<p>Điểm '.round(($score/$total_questions)*10,2).' / 10</p>';
